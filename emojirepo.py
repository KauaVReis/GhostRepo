...
# -*- coding: utf-8 -*-
"""
EmojiRepo CLI - Ferramenta de Esteganografia Unicode
...
def log_error(msg):
    print(f"\033[91m[-] Erro: {msg}\033[0m", file=sys.stderr)
...
def decode_invisible_to_bytes(text: str) -> bytes:
    ...
    if len(nibbles) % 2 != 0:
        raise ValueError("Payload corrompido: número ímpar de caracteres invisíveis.")
...
def download_github_zip(repo_url: str) -> bytes:
    ...
    # Extrair usuario e repositorio da URL
    clean_url = repo_url.strip().rstrip('/')
    parts = clean_url.split('github.com/')
    if len(parts) < 2:
        raise ValueError("URL do GitHub inválida. Formato esperado: https://github.com/usuario/repositorio")
        ...
        try:
            with urllib.request.urlopen(req) as response:
                if response.status == 200:
                    ...
                    if len(zip_data) > 100:
                        log_success(f"Branch '{branch}' baixada com sucesso ({len(zip_data)} bytes).")
                        return zip_data
...
def run_decode(input_file: str, output_zip: str, extract: bool = False):
    ...
    if calculated_sha == expected_sha:
        log_success("Verificação de Integridade: OK (Checksum bate perfeitamente!)")
    else:
        log_error("ALERTA: O hash do arquivo decodificado difere do original. Arquivo corrompido!")
    ...
    if extract:
        ...
        dest_dir = output_zip[:-4] if output_zip.lower().endswith('.zip') else output_zip + "_extracted"
        try:
...
def interactive_menu():
    while True:
        print("\n\033[92m" + "="*50)
        print("              EMOJIREPO INTERACTIVO               ")
        print("="*50 + "\033[0m")
        print(" 1 - Gerar Payload Stegano (Encode)")
        print(" 2 - Restaurar Projeto ZIP (Decode)")
        print(" 3 - Sair")
        ...
        if choice == '1':
            source = input("\nDigite a URL do repositório GitHub ou caminho do ZIP local: ").strip()
            if not source:
                print("\033[91mFonte inválida!\033[0m")
                continue
            
            # Autocompletar extensão de ZIP local por conveniência
            if not source.startswith("http://") and not source.startswith("https://"):
                if not source.lower().endswith('.zip') and not os.path.exists(source):
                    source += '.zip'
                
            output = input("Digite o nome do arquivo de saída [.txt] (padrão: emoji.txt): ").strip()
            if not output:
                output = "emoji.txt"
            elif not output.lower().endswith('.txt'):
                output += '.txt'
                
            compress_opt = input("Ativar compressão GZip? (S/n): ").strip().lower()
            use_compress = compress_opt != 'n'
            
            extreme_opt = input("Ativar Modo Invisível Extremo (sem emoji âncora)? (s/N): ").strip().lower()
            ...
            input_file = input("\nDigite o caminho do arquivo de entrada [.txt] (padrão: emoji.txt): ").strip()
            if not input_file:
                input_file = "emoji.txt"
            elif not input_file.lower().endswith('.txt'):
                input_file += '.txt'
                
            output_zip = input("Digite o nome do ZIP de saída (padrão: projeto_restaurado.zip): ").strip()
            if not output_zip:
                output_zip = "projeto_restaurado.zip"
            elif not output_zip.lower().endswith('.zip'):
                output_zip += '.zip'
                
            extract_opt = input("Deseja extrair (unzip) os arquivos do ZIP restaurado? (S/n): ").strip().lower()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="EmojiRepo CLI - Codifique e Decodifique ZIPs e repositórios GitHub em emojis invisíveis.")
    subparsers = parser.add_subparsers(dest="command", help="Comandos disponíveis")

    # Parser do comando encode
    encode_parser = subparsers.add_parser("encode", help="Esconde um ZIP ou repositório GitHub atrás de caracteres invisíveis.")
    encode_parser.add_argument("source", help="URL do repositório GitHub público OU caminho de um arquivo .zip local.")
    encode_parser.add_argument("-o", "--output", default="emoji.txt", help="Caminho do arquivo .txt de saída (padrão: emoji.txt).")
    encode_parser.add_argument("--no-compress", action="store_true", help="Desativa a compressão Gzip do payload.")
    encode_parser.add_argument("--extreme", action="store_true", help="Ativa o modo invisível extremo (sem emoji visível de âncora).")

    # Parser do comando decode
    decode_parser = subparsers.add_parser("decode", help="Restaura um arquivo ZIP a partir de um arquivo de esteganografia.")
    decode_parser.add_argument("input", help="Caminho do arquivo .txt esteganográfico contendo o payload.")
    decode_parser.add_argument("-o", "--output", default="projeto_restaurado.zip", help="Caminho do arquivo .zip de saída (padrão: projeto_restaurado.zip).")
    decode_parser.add_argument("-x", "--extract", action="store_true", help="Extrai (unzip) o arquivo ZIP resultante em uma pasta.")

    args = parser.parse_args()

    if args.command == "encode":
        run_encode(args.source, args.output, not args.no_compress, args.extreme)
